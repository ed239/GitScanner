...
const PieChart = ({
  ...
  setChartImageCommitsPie,
  setChartImagePullsPie,
}) => {
  ...
  const sortedPRContributors = Object.keys(pullRequestsByContributor).sort(
  ...
  const data = {
    ...
    datasets: [
      {
        label: 'Contributions',
        data: sortedContributors.map(
          (contributor) => contributor.contributions
        ),
        backgroundColor: sortedContributors.map(
          (_, index) => colors[index % colors.length]
        ),
        hoverBackgroundColor: sortedContributors.map(
          (_, index) => colors[index % colors.length]
        ),
      },
    ],
  ...
  const dataPR = {
    ...
    datasets: [
      {
        label: 'Pull Requests',
        data: sortedPRContributors.map(
          (login) => pullRequestsByContributor[login]
        ),
        backgroundColor: sortedPRContributors.map(
          (_, index) => colors[index % colors.length]
        ),
        hoverBackgroundColor: sortedPRContributors.map(
          (_, index) => colors[index % colors.length]
        ),
      },
    ],
